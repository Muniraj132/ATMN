...
<div class="col-md-9">
      ...
      @if(isset($data))
          <form  action="{{ url('/recruitment/brief/dsupdate', ATMNHelper::encryptUrl(@$data->id)) }}"  role="form" method="POST" class="php-email-form">
      @endif
      @csrf
     ...
     <div class="row">
        ...
        <div class="col-md-12">
          <div class="row">
            ...
            <div class="form-group col-md-6" align="end">
              ...
              <label class="label-name">
                <strong>
                <input type="text" name="pastor" class="underline bri-date" id="pastor" value="{{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s',$data->created_at)->format('m-d-Y') }}" disabled="disabled">
                </strong>
              </label>
            </div>
          </div>
        ...
        <div class="col-md-12">
          <div class="row">
            ...
            <div class="form-group col-md-6" align="end">
              ...
              <input type="text" name="church_name" class="underline church-name" id="church_name" value="{{ $data->church_name }}" disabled="disabled">
            </div>
          </div>
        </div>
      </div>  
        ...
        <div class="col-md-12">
          <div class="row">
            ...
            <div class="col form-group col-md-12">
              <textarea class="form-control" name="ds_comments" placeholder="DS Comments" rows="4">{{$data->ds_comments ?? '' }}</textarea>
            </div>
          </div>
        </div>
      </div>
      ...
      @endif
    	...
    	@if($isUSER=='ds')
          ...
          <input type="hidden" name="brief_status" value="Approved">
          <input type="hidden" name="ds_approved_date" value="{{ Carbon::now() }}">
          <button type="submit">{{ trans('main.approve') }}</button>
          <button class="exit"><a href="{{ url('recruitment/brief-dspending-list')}}" style="color:#fff">Close</a></button>
    	@endif
      </div>
      </form>
      </div>
