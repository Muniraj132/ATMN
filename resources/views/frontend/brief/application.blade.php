...
}elseif(request()->is('*/edit/*') && $brief_status == 'Approved'){
  $url = 'disabled="disabled"';
}else{
  $url = '';
}
...
<div class="col-md-9">
    ...
    @if(isset($data))
        <form  action="{{ url('/recruitment/brief/update', ATMNHelper::encryptUrl(@$data->id)) }}"  role="form" method="post" class="php-email-form">
     ...
     <div class="row">
      ...
      <div class="row" style="height: 30px;">
        ...
        <div class="form-group col-md-6" align="end">
          ...
          <label class="label-name"><strong><input type="text" name="applydate" class="underline bri-date" id="pastor" value="{{Carbon\Carbon::now()->format('m-d-Y')}}" style="background-color: #f6f9fe;"></strong></label>
        </div>
      </div> 
      ...
      <div class="row">
        <div class="form-group col-md-6">
          ...
          <select class="select-district" name="d_id" disabled="disabled">
            <option>Select District</option>
              @foreach($district as $value)
              <option value="{!! $value->d_id !!}" {{ $atmnData->district_id == $value->d_id ? 'selected' : '' }}>{!! $value->district !!}</option>
              @endforeach
          </select>
          <input type="hidden" name="d_id" value="{!! $atmnData->district_id !!}">
        </div>
        <div class="form-group col-md-6" align="end">
          <label class="label-name">Church:</label>
          <input type="text" name="church_name" class="underline church-name" id="church_name" value="{{$data->church_name ?? '' }}" {{ $url }}>
        </div>
      </div>  
        ...
        <div class="col-md-12">
          <div class="row">
            ...
            <div class="form-group col-md-12 ">
              ...
              <textarea class="form-control" name="personal_wellbeing_are_you_exited" {{ $url }}>{{$data->personal_wellbeing_are_you_exited ?? '' }}</textarea>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="col-md-12">
          <div class="row">
            ...
            <div class="form-group col-md-12 ">
              ...
              <textarea class="form-control" name="overview_church_health_prayer_attention" {{ $url }}>{{$data->overview_church_health_prayer_attention ?? '' }}</textarea>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="col-md-12">
          <div class="row">
            ...
            <div class="form-group col-md-12 spec-concern">
              ...
              <textarea class="form-control" name="specific_concern_restoring_hope" {{ $url }}>{{$data->specific_concern_developing_culture_of_prayer ?? ''}}</textarea>
              ...
              <div class="sub-">
                ...
                <textarea class="form-control" name="specific_concern_developing_culture_of_prayer" {{ $url }}>{{$data->specific_concern_developing_culture_of_prayer ?? ''}}</textarea>
              </div>
              ...
              <textarea class="form-control" name="specific_concern_rebuilding_mission" {{ $url }}>{{$data->specific_concern_rebuilding_mission ?? ''}}</textarea>
              ...
              <textarea class="form-control" name="specific_concern_recruit_lay_leaders" {{ $url }}>{{$data->specific_concern_recruit_lay_leaders ?? ''}}</textarea>
              ...
              <textarea class="form-control" name="specific_concern_reset_governance"  {{ $url }}>{{$data->specific_concern_reset_governance ?? ''}}</textarea>
              ...
              <textarea class="form-control" name="specific_concern_resolve_conflict" {{ $url }}>{{$data->specific_concern_resolve_conflict ?? ''}}</textarea>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="col-md-12">
          <div class="row">
            ...
            <div class="form-group col-md-12 ">
             ...
             <textarea class="form-control" name="other_objectives_outline" {{ $url }}>{{$data->other_objectives_outline ?? '' }}</textarea>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="row">
        <div class="col-md-12">
          <div class="row">
            ...
            <div class="form-group col-md-12 ">
              <textarea class="form-control" name="greatest_priority_for_next_month"  {{ $url }}>{{$data->greatest_priority_for_next_month ?? '' }}</textarea>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="row">
        <div class="col-md-12">
          <div class="row">
            ...
            <div class="form-group col-md-12 ">
              <textarea class="form-control" name="mou_with_church" {{ $url }}>{{$data->mou_with_church ?? '' }}</textarea>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="col-md-12">
          <div class="row">
            ...
            <div class="col form-group col-md-12">
              <textarea class="form-control" name="ds_comments" placeholder="DS Comments" rows="4" {{ $url }}>{{$data->ds_mou_with_church ?? '' }}</textarea>
            </div>
          </div>
        </div>
      </div>
      ...
      @if($isUSER=='')
        <input type="hidden" name="brief_status" value="Pending">
        @if(request()->is('*/create'))
      	  <button type="submit">{{ trans('main.save') }}</button>
        @endif
        @if($brief_status == 'Pending')
          @if(request()->is('*/edit*'))
         	  <button type="submit">{{ trans('main.update') }}</button>
          @endif
        @endif
       <a href="{{Route('brief.index')}}" style="color: #fff; background: #801214; padding: 11px 22px 11px 21px; border-radius: 5px;"onmouseover="">Close</a>
      @endif
