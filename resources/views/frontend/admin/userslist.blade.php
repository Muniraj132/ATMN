                ...
                <tr id="user_row{{$value['id']}}">
                    ...
                    <td style="width: 70px;">
                        <label id="mybtn" data-id="{{$value['id']}}" data-toggle="tooltip" title="Assign Role">
                            <span  class="user-type" id="user_type{{$value['id']}}" style="color:#801214">{{ $user_type }}</span>
                        </label>
                        @if($value["requestforstaff"]=="Y" && $user_type == "Pastor") 
                        <i class="fas fa-info-circle" id="requestforstaff" data-toggle="tooltip" title='Requested "D.S. or N.O." Role {{ ATMNHelper::getdistrictname(@$value["requeststaffdistrict"])}}'></i> 
                        @endif
                    </td>
