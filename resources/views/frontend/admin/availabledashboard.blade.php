    ...
    <nav id="navbar" class="flexbox profile-nav">
      ...
      <div class="navbar-inner view-width flexbox-space-bet">
        <div class="row align-middle">
            <h4 class="align-middle">@if($user_type == "National Office") National Office @else Admin @endif Dashboard (Pastor Availability)</h4>
        </div>         
        <br>
      </div>
    </nav>
            ...
            <tr>
                <td>
                    ...
                    @if($value["placed"]==0)
                        <td>-</td>
                    @else
                        <td><a href="{{route('app.districtplacedlist',array(ATMNHelper::encryptUrl(@$value['district_id'])))}}">{{$value["placed"]}}</a></td>
                    @endif
                    @if($value["available"]==0)
                        <td>-</td>
                    @else
                        <td><a href="{{route('app.districtavailablelist',array(ATMNHelper::encryptUrl(@$value['district_id'])))}}">{{$value["available"]}}</a></td>
                    @endif
            </tr>
        @endforeach
        </tbody>
